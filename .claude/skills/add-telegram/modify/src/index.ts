...
function ensureDockerRunning(): void {
  ...
  try {
    ...
    if (orphans.length > 0) {
      try {
        execSync(`docker stop ${orphans.join(' ')}`, { stdio: 'pipe', timeout: 15000 });
      } catch { /* some may already be stopped */ }
      try {
        execSync(`docker rm ${orphans.join(' ')}`, { stdio: 'pipe' });
      } catch { /* some may already be removed */ }
      logger.info({ count: orphans.length, names: orphans }, 'Cleaned up orphaned containers');
    }
  } catch (err) {
