...
`.nanoclaw/base/` holds a clean copy of the core codebase at the current installed version. This is the single common ancestor for all three-way merges.

- All skills are authored against the clean core
- The base is **only updated during core updates**, never after applying a skill
- `git merge-file` always uses this as the common ancestor
...
# Compatibility
core_version: 0.5.0               # The core version this skill was authored against
min_skills_system_version: "0.1.0" # Minimum skills system version required
...
tested_with:               # Skills confirmed to stack cleanly
  - telegram@1.0.0
  - slack@1.1.0

# Commands to run after successful apply (for non-structured operations only)
post_apply: []

# Test command
test: "npm test -- --grep whatsapp"
```
...
1. Pre-flight checks
2. **File operations** (renames, deletes, moves)
3. Copy new files
4. Merge modified code files
5. Conflict resolution
6. Apply structured operations (batched)
7. Run `npm install` (once, if any structured npm_dependencies exist)
8. Post-apply commands (non-structured only)
9. Update state
10. Run tests
11. Clean up (delete backup on success, restore on failure)
...
last_updated: 2026-02-15T14:20:00Z

# Path remapping from core updates
path_remap: {}
...
applied_skills:
  ...
  - skill: telegram
    version: 1.0.0
    applied_at: 2026-02-15T11:05:00Z
    core_version_at_apply: 0.5.0
    custom_patch: .nanoclaw/custom/telegram-group-only.patch
    custom_patch_description: "Restrict bot responses to group chats only"
    files_added:
  ...
  - skill: telegram-reactions
    version: 1.0.0
    applied_at: 2026-02-15T11:10:00Z
    core_version_at_apply: 0.5.0
    files_modified:
      - src/channels/telegram.ts
    file_hashes:
...
Storing base, skill, and merged hashes per file makes drift diagnosis instant. Storing structured outcomes makes structured operations replayable and auditable.
...
5. **One shared base.** `.nanoclaw/base/` is the single common ancestor. Only updated on core updates.
